...
import utils as ut
# Initializing OpenAI client with base URL and API key
client = OpenAI(base_url="https://api.groq.com/openai/v1",
                api_key=os.environ['GROQ_API_KEY'])

# Function to generate a personalized email based on churn probability and customer data
...
# Function to explain the churn prediction in customer-friendly language
...
# Loading various models for churn prediction
...
# Function to prepare customer data input for the ML models
def prepare_input(credit_score, location, gender, age, tenure, balance,
                  num_products, has_credit_card, is_active_member,
                  estimated_salary):
    # Creating dictionary for customer data
    input_dict = {
...
# Function to predict churn probability and display it in charts
def predict_churn(input_df, input_dict):
  # Calculating churn probability from multiple models
    probabilities = {
    ...
    # Displaying probability charts in Streamlit columns
...
# Streamlit app setup and customer data input
st.title("Customer Churn Prediction")
df = pd.read_csv("churn.csv")  # Loading customer data from CSV
customers = [
    f"{row['CustomerId']} - {row['Surname']}" for _, row in df.iterrows()
]
selected_customer_option = st.selectbox("Select a customer", customers)
# Initializing variables for customer input
credit_score = None
location = None
gender = None
age = None
...
# Collecting customer inputs in Streamlit
if selected_customer_option:
    ...
    with st.columns(1)[0]:
       ...
       # Selecting an LLM model for generating explanations and emails
        available_llms = [
        ...
        # Preparing input and predicting churn
